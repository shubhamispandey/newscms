                ...
                <?php
                    ...
                    if(isset($_GET['aid'])){
                        ...
                        $sql = "SELECT * FROM post 
                                ...
                                WHERE user_id = {$aid}
                                LIMIT {$offset}, {$limit}";
                        $sql1 = "SELECT * FROM user WHERE user_id = {$aid}";
                    }else{
                        $page_no = 1;
                        $sql = "SELECT * FROM post 
                            LEFT JOIN user ON user.user_id = post.author
                ...
                <div class="post-container">
                    ...
                    <ul class='pagination'>
                        <?php
                            ...
                            if($page_no > 1){
                                $page_prev = $page_no - 1;
                                echo '<li><a href="https://localhost/NEWScms/author.php?page='.$page_prev.'&aid='.$aid.'">'.'Prev'.'</a></li>';
                            }
                            for($i=1; $i<= $pages; $i++){
                                if($page_no == $i){
                                    $active = "class='active'";
                                ...
                                echo '<li '.$active.'><a '.$style.'href="https://localhost/NEWScms/author.php?page='.$i.'&aid='.$aid.'">'.$i.'</a></li>';
                            }
                            if($page_no < $pages){
                                $page_next = $page_no + 1;
                                echo '<li><a href="https://localhost/NEWScms/author.php?page='.$page_next.'&aid='.$aid.'">'.'Next'.'</a></li>';
                            }
